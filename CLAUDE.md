...
AUDIO_PRIME is a professional real-time audio spectrum analyzer built with Electron 35, Svelte 5, TypeScript, and WebGL2. It features studio-quality metering (LUFS, VU, true peak), beat detection, voice analysis, and Spotify integration.
...
### Process Separation
- **Electron Main** (`electron/main.ts`): IPC handlers, audio capture via `parec` subprocess, Spotify OAuth with PKCE, window management
- **Electron Preload** (`electron/preload.ts`): Context bridge exposing `window.electronAPI`
- **Renderer** (`src/`): Svelte 5 SPA with WebGL2 rendering
...
- `src/rendering/renderers/SpectrumRenderer.ts` - WebGL2 spectrum visualization
- `src/components/panels/` - SpectrumPanel, BassDetailPanel, DebugPanel
